...
export function Bar3DChartConfig({ sheetData, onPreviewChange }: Bar3DChartConfigProps) {
  ...
  useEffect(() => {
    ...
    const barData = formattedRows.map(row => [
    ...
    const config = {
      title: { text: title || '3D Bar Chart', left: 'center' },
      tooltip: {},
      xAxis3D: { type: 'value' },
      yAxis3D: { type: 'value' },
      zAxis3D: { type: 'value' },
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Z-axis (Height) Column</label>
        <select
          value={zColumn}
      ...
      <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
        <p className="text-sm text-blue-800">
          💡 3D bar visualization with X, Y position and Z height
        </p>
      </div>
    </div>
  );
